...
async function getWinOfficeInfo() {
    try {
        ...
        const computerInfo = await new Promise((resolve, reject) => {
            exec(
                'powershell -Command "Get-ComputerInfo | Select-Object OsName,OSDisplayVersion,WindowsEditionId,OsVersion | ConvertTo-Json"',
                { encoding: 'utf8' },
                (error, stdout, stderr) => {
                    if (error) {
                        reject(error);
        ...
        const ActivationStatut = await new Promise((resolve, reject) => {
            exec(
                `powershell -Command "Get-CimInstance SoftwareLicensingProduct -Filter \\"Name like 'Windows%'\\" | where { $_.PartialProductKey } | select Description, LicenseStatus | ConvertTo-Json"`,
                { encoding: 'utf8' },
                (error, stdout, stderr) => {
                    if (error) {
                        reject(error);
