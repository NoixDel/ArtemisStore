...
function parseWingetOutput(rawOutput) {
    ...
    for (let i = headerIndex + 1; i < lines.length; i++) {
        ...
        if (/^\d+ mises? a niveau/.test(line)) continue;
...
async function getUpgradableApps() {
    ...
    logger.info('[UpdatesApps.js] Execution de : ' + command);
    ...
    try {
        ...
        logger.info(`[UpdatesApps.js] ${updates.length} mise(s) a jour trouvee(s).`);
        return updates;
    } catch (err) {
...
function UpdatesAppsListener() {
    ...
    ipcMain.on('update-all-apps', async () => {
        const win = BrowserWindow.getFocusedWindow();
        if (!win) return logger.error('[UpdatesApps.js] Aucune fenetre active.');

        try {
            await runAllWingetUpdates((type, data) => {
                win.webContents.send('install-progress', { app_id: 'ALL', data });
            });
        } catch (err) {
            logger.error('[UpdatesApps.js] Echec mise a jour globale : ' + err.message);
            win.webContents.send('install-progress', { app_id: 'ALL', data: err.message });
        }
    });
}

async function runAllWingetUpdates(onProgress = null, useUac = true) {
    logger.info('[UpdatesApps.js] Mise a jour de toutes les applications : ' + UPDATE_ALL_COMMAND);

    return runcmd(UPDATE_ALL_COMMAND, useUac, useUac, onProgress);
}
